#include<stdio.h>
#include<stdlib.h>
struct listnode { // สร้าง struct ชื่อ listnode
	int data; // กำหนดตัวแปร
	struct listnode *next; // กำหนดตัวไว้สำหรับชี้ Node
};
typedef struct listnode LN; // ย่อชื่อ struct listnode

void insert_at_front(LN **hptr, int d); // กำหนด Pototype สำหรับเชื่อม Node
...
int main(){
	...
	do{
		...
		if(d > 0){ // เข้าเงื่อนไข if d > 0
			insert_at_front(&head, d); // ส่งที่อยู่ของ head กับ d ไปในฟังก์ชัน insert_at_front
		}
