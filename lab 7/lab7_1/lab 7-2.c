...
int length1(LISTP lst)
{
    int n; // กำหนด int n ไว้นับ

    for (n = 1; lst->nextptr != NULL; n++) // วน ใน for loop ถ้า lst->nextpter ไม่เท่ากับ NULL จะวนเลื่อยๆ
        lst = lst->nextptr;

    return n; // return ค่า n ไปในฟังก์ชันที่เรียกใช้
}

//  return the number of elements in the list -> recurse function
int length2(LISTP lst)
{
    if (lst->nextptr == NULL) // ถ้าเข้าเงื่อนไข  จะส่งค่า 1 ไปในฟังชันที่เรียกใช้
        return 1;
    else // ถ้าไม่เข้าเงื่อนไข if ทำการ recurse ไปเรื่อยๆจนกว่าเจอเงื่อนไขที่หยุด
