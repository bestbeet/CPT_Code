...
int sumList(LISTP lst)
{
    LISTP curr; // สร้าง curr ขึ้นมา
    int sum = 0; // กำหนด int sum มาเพื่อเก็บค่า

    curr = lst; // ให้  curr = lst

    while (curr->nextptr != NULL) // ทำการวนloop-while จนกว่าจะเจอ node สุดท้าย
    {
        sum += curr->data; // sum ทำการบวกค่าที่เก็บใน curr->data
        curr = curr->nextptr; // ให้ค่า curr ชี้ไปที่ node ถัดไป
    }

    return sum; // ส่งค่า sum ไปยังฟังก์ช้นที่เรียกใช้งาน
}
